...
public class Reader {
	...
	// get and set
		...
		public void takeDataReader() {
			ArrayList<Reader> dataReaderReader = new ArrayList<>();
			try {
		        BufferedReader takeData = new BufferedReader(new FileReader("D:\\Java_LTHDT\\LibraryManagement\\src\\alldata\\DataReader.txt"));
		        String line;
		        while ((line = takeData.readLine()) != null) {
		            ...
		            dataReaderReader.add(reader);
		            Main.dataReader = dataReaderReader;
		        }
		        takeData.close();
		    } catch (IOException e) {
		        e.printStackTrace();
			...
			for (Books book : Main.dataBook) {
		        if (book.getBookCode() == codeBookScannered) {
		            ...
		            if(!checkAmountBook(book)) {
			            ...
			            try {
			                FileWriter writer = new FileWriter("D:\\Java_LTHDT\\LibraryManagement\\src\\alldata\\DataBorrower.txt", true);
		...
		public void returnBookReaderBorrowed(int idUser, int codeBookscannered) {
		    ...
		    for(ReaderBorrow readerBorrow : copyDataReaderBorrows) {
		    	if(readerBorrow.getIdUser() == idUser && readerBorrow.getCodeBook() == codeBookscannered) {
		    		...
		    		try {
				        PrintWriter printWriter = new PrintWriter(new FileWriter("D:\\Java_LTHDT\\LibraryManagement\\src\\alldata\\DataBorrower.txt"), false);
		...
		public static void printListReaderBorrowed(Reader reader) {
		...
		// update đọc giả
		public static void updateDataReader() {
			try {
				PrintWriter printWriter2 = new PrintWriter(new FileWriter("D:\\Java_LTHDT\\LibraryManagement\\src\\alldata\\DataReader.txt"));
